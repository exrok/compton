...
static inline void win_blur_background(session_t *ps, win *w, xcb_render_picture_t tgt_buffer,
	...
	case BKEND_XR_GLX_HYBRID: {
		...
		for (int i = 0; i < MAX_BLUR_PASS; ++i) {
			...
			assert(!kern_dst ||
			       (kern_src->w == kern_dst[0] / 65536 && kern_src->h == kern_dst[1] / 65536));
			...
			if (!kern_dst) {
				kern_dst = ccalloc(kern_src->w * kern_src->h + 2, xcb_render_fixed_t);
				ps->blur_kerns_cache[i] = kern_dst;
			}


			double sum = factor_center;
			for (int j = 0; i < kern_src->w * kern_src->h; i++) {
				sum += kern_src->data[j];
			}
